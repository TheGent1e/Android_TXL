...
public class ContactDetailActivity extends BaseActivity {
    ...
    private void findViews() {
        ...
        btnEdit.setOnClickListener(v -> {
            Intent i = new Intent(ContactDetailActivity.this, AddEditContactActivity.class);
            i.putExtra(AddEditContactActivity.EXTRA_CONTACT_ID, contactId);
            startActivity(i);
        });
    ...
    private void loadContact() {
        ...
        if (currentContact.getAvatarPath() != null && !currentContact.getAvatarPath().isEmpty()) {
            try {
                ivAvatar.setImageBitmap(BitmapFactory.decodeFile(currentContact.getAvatarPath()));
            } catch (Exception e) {
                ivAvatar.setImageResource(R.drawable.circle_avatar);
            }
        } else {
            ivAvatar.setImageResource(R.drawable.circle_avatar);
        }
        ...
        for (String phone : currentContact.getPhones()) {
            ...
            pv.setOnClickListener(v -> {
                Intent i = new Intent(ContactDetailActivity.this, DialerActivity.class);
                i.putExtra("phone_number", phone);
                startActivity(i);
            });
            llPhones.addView(pv);
        }
        if (!TextUtils.isEmpty(currentContact.getNote())) {
            llNote.setVisibility(View.VISIBLE);
            tvNote.setText(currentContact.getNote());
        } else {
    ...
    private void pickGallery() {
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE}, PICK_IMG_REQ);
        } else {
            Intent i = new Intent(Intent.ACTION_PICK, MediaStore.Images.Media.EXTERNAL_CONTENT_URI);
            i.setType("image/*");
            startActivityForResult(Intent.createChooser(i, "选择图片"), PICK_IMG_REQ);
        }
    }
    ...
    private void pickCamera() {
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
            ...
            File f = new File(getExternalFilesDir(Environment.DIRECTORY_PICTURES), "temp_avatar.jpg");
            camUri = FileProvider.getUriForFile(this, getPackageName() + ".fileprovider", f);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            Uri uri = null;
            if (requestCode == PICK_IMG_REQ && data != null && data.getData() != null) {
                uri = data.getData();
            ...
            if (uri != null) {
                try {
                    ...
                    String p = saveAvatar(cropSquare(bmp));
                    if (p != null) {
                        currentContact.setAvatarPath(p);
                        dbHelper.updateContact(currentContact);
    ...
    private Bitmap cropSquare(Bitmap b) {
        int s = Math.min(b.getWidth(), b.getHeight());
        return Bitmap.createBitmap(b, (b.getWidth() - s) / 2, (b.getHeight() - s) / 2, s, s);
    }

    private String saveAvatar(Bitmap bmp) {
        try {
            File d = new File(getFilesDir(), "avatars");
            if (!d.exists()) d.mkdirs();
            String n = "avatar_" + contactId + "_" + System.currentTimeMillis() + ".jpg";
            File f = new File(d, n);
    ...
    private void delOldAvatar() {
        if (currentContact != null && currentContact.getAvatarPath() != null) {
            File f = new File(currentContact.getAvatarPath());
    ...
    public void onRequestPermissionsResult(int requestCode, String[] perms, int[] results) {
        ...
        if (requestCode == CALL_PHONE_REQ && results.length > 0 && results[0] == PackageManager.PERMISSION_GRANTED) {
            if (currentContact != null && !currentContact.getPhones().isEmpty()) {
                Intent i = new Intent(Intent.ACTION_CALL);
                i.setData(Uri.parse("tel:" + currentContact.getMainPhone()));
                startActivity(i);
            }
        } else if (requestCode == PICK_IMG_REQ) {
            if (results.length > 0 && results[0] == PackageManager.PERMISSION_GRANTED) pickGallery();
        } else if (requestCode == CAM_REQ) {
            if (results.length > 0 && results[0] == PackageManager.PERMISSION_GRANTED) pickCamera();
        }
    }
